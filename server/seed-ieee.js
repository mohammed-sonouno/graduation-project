 ...
 * - eng.dean@najah.edu (dean, Faculty of Engineering)
 * - IEEE community → Faculty of Engineering (IEEE governance / dean approval)
 */
...
const ENG_NAME = 'Faculty of Engineering';
const IT_NAME = 'Faculty of Information Technology & Artificial Intelligence';
...
async function seedIeee() {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO colleges (name) SELECT $1 WHERE NOT EXISTS (
         SELECT 1 FROM colleges WHERE lower(trim(name)) = lower(trim($1))
       )`,
      [ENG_NAME]
    );
    await client.query(
    ...
    if (engRow.rows.length === 0) {
      throw new Error(`Missing colleges row "${ENG_NAME}". Run: npm run migrate`);
    }
    const engId = engRow.rows[0].id;
    ...
    try {
      await client.query(`UPDATE communities SET kind = 'association' WHERE id = $1`, [ieeeId]);
    } catch (_) {
      /* ignore if kind column missing */
    }
    try {
      await client.query(`UPDATE communities SET colleges = $1::text[] WHERE id = $2`, [[ENG_NAME], ieeeId]);
    } catch (e) {
      if (e.code !== '42703') throw e;
    }
    console.log(`Community created: IEEE (id ${ieeeId}, college_id ${engId} — ${ENG_NAME})`);
    ...
    if (r.rows.length > 0) {
      await client.query(
        ...
        [deanEmail, passwordHash, engId, r.rows[0].id]
      );
      console.log(`Engineering dean updated: eng.dean@najah.edu → ${ENG_NAME}, password 123`);
    } else {
      await client.query(
        "INSERT INTO app_users (email, password_hash, role, college_id) VALUES ($1, $2, 'dean', $3)",
        [deanEmail, passwordHash, engId]
      );
      console.log(`Engineering dean created: eng.dean@najah.edu → ${ENG_NAME}, password 123`);
    }

    const up = await client.query('UPDATE events SET community_id = $1 WHERE community_id IS NULL RETURNING id', [
      ieeeId,
    ]);
    if (up.rowCount > 0) {
