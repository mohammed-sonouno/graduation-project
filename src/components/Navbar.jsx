...
function Navbar() {
  ...
  useEffect(() => {
    if (!user) {
      setNotifications([]);
      notificationsFetchedForRef.current = null;
      return;
    }
  ...
  const avatarSrc = useMemo(
    () => (user?.picture ? eventImageUrl(user.picture) : null),
    [user?.picture]
  );
  ...
  const visibleCenterLinks = centerLinks.filter(
    (link) => !(link.staffOnly && !isStaffUser)
  );
            ...
            {user ? (
              <>
                ...
                <button
                  ...
                  className="relative h-10 w-10 rounded-full bg-blue-900 text-white font-semibold flex items-center justify-center overflow-hidden cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-900/30 focus:ring-offset-2"
                  aria-expanded={open}
                  aria-haspopup="true"
                ...
                {open && (
                  ...
                  >
                    <div className="px-4 py-3 flex items-center gap-3 border-b border-slate-100">
                      <div className="relative flex-shrink-0 h-10 w-10 rounded-full bg-blue-900 text-white font-semibold flex items-center justify-center text-sm overflow-hidden">
                        {avatarSrc && !avatarImageError ? (
                          <img
                            src={avatarSrc}
                            alt=""
