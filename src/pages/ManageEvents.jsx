...
const STATUS_STYLES = {
  ...
  pending_dean: 'bg-amber-100 text-amber-800',
...
const isDraft = (ev) => String(ev?.status ?? '').toLowerCase() === 'draft';

/** True when event was rejected by supervisor (0), dean (1), or admin (2). Backend sets status=draft and rejected_at_step. */
function isRejected(ev) {
  if (!ev || String(ev?.status ?? '').toLowerCase() !== 'draft') return false;
  const step = Number(ev.rejectedAtStep);
  return !Number.isNaN(step) && step >= 0 && step <= 2;
}

/** Normalize rejected_at_step to 0, 1, or 2 for the stepper (handles string from DB). */
function normalizeRejectedStep(value) {
  const n = Number(value);
  if (Number.isNaN(n) || n < 0 || n > 2) return undefined;
  return n;
}

function getStatusLabel(ev) {
  if (isRejected(ev)) return 'REJECTED';
  return STATUS_LABELS[ev?.status] || 'DRAFT';
}

function getStatusStyle(ev) {
  if (isRejected(ev)) return STATUS_STYLES.rejected;
  return STATUS_STYLES[ev?.status] || STATUS_STYLES.draft;
}

function EventCard({ ev, selectedId, setSelectedId, setShowForm, onDelete, onDeleteDraft, onDeleteDraftConfirm, events, onShowFeedback, isCommunityLeaderOnly }) {
  return (
    <div
      className={`relative rounded-2xl border overflow-hidden bg-white shadow-sm transition-all duration-200 min-w-0 ${
    ...
    >
      <div className="relative aspect-[4/3] w-full bg-slate-100">
        ...
        />
        <span
          className={`absolute left-2 top-2 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${getStatusStyle(ev)}`}
        >
          {getStatusLabel(ev)}
        </span>
      </div>
      <div className="p-4">
        ...
        <SmallApprovalStepper
          status={ev.status}
          approvalStep={ev.approvalStep}
          rejectedAtStep={isRejected(ev) ? normalizeRejectedStep(ev.rejectedAtStep) : undefined}
          requestedChangesAtStep={ev.requestedChangesAtStep}
        />
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {isDraft(ev) && !isRejected(ev) && (
            <>
              ...
              <button
                type="button"
                onClick={() => {
                  if (onDeleteDraft) onDeleteDraft(ev.id);
                  else if (window.confirm('Delete this draft event?')) {
                    onDelete?.(ev);
                    if (selectedId === ev.id) setSelectedId(null);
                  }
                }}
          ...
          {(ev.status === 'changes_requested' || ev.status === 'needs_changes') && (
            <>
              {isCommunityLeaderOnly && (
                <button
                  type="button"
                  onClick={() => { setSelectedId(ev.id); setShowForm(true); }}
                  className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-[#00356b] transition-colors hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#00356b]/20"
          ...
          {ev.status === 'rejected' && !isRejected(ev) && (
            <button
              type="button"
              onClick={() => {
                if (window.confirm('Remove this rejected event from the list?')) {
...
function ManageEvents() {
  ...
  const accessAllowed = canAccessManageEvents(user);
  const allowCreateEvents = canCreateEvents(user);
  ...
  const getInitialFormState = useCallback(() => ({
    ...
    price: '',
    priceMember: '',
    communityId: isLeader && user?.community_id != null ? String(user.community_id) : '',
    customSections: [],
    forAllColleges: true,
    targetCollegeIds: [],
    targetAllMajors: true,
    targetMajorIds: [],
  }), [isLeader, user?.community_id]);
  ...
  useEffect(() => {
    if (!loading && user && !accessAllowed) navigate('/admin', { replace: true });
  ...
  const handleDeleteDraft = useCallback(async (id) => {
    ...
    try {
      ...
      if (err?.data?.error) window.alert(err.data.error);
    }
  }, []);

  const handleConfirmDeleteDraft = useCallback(() => {
    if (!draftToDelete) return;
    const ev = draftToDelete;
  ...
  }, [draftToDelete, loadEvents, selectedId]);
  ...
  useEffect(() => {
    ...
    getEvent(selectedId)
      .then((full) => {
        if (cancelled || !full || String(full.id) !== String(selectedId)) return;
        const targetCollegeIds = Array.isArray(full.targetCollegeIds) ? full.targetCollegeIds.map((id) => String(id)) : [];
        const targetMajorIds = Array.isArray(full.targetMajorIds) ? full.targetMajorIds.map((id) => String(id)) : [];
        setForm({
          title: full.title || '',
          ...
          availableSeats: full.availableSeats != null ? String(full.availableSeats) : '',
          price: full.price != null ? String(full.price) : '',
          priceMember: full.priceMember != null ? String(full.priceMember) : '',
          communityId: full.communityId != null ? String(full.communityId) : '',
          customSections: Array.isArray(full.customSections) ? full.customSections.map((s) => ({ ...s })) : [],
          forAllColleges: full.forAllColleges !== false,
          targetCollegeIds,
          targetAllMajors: full.targetAllMajors !== false,
          targetMajorIds,
        });
      })
      .catch(() => {});
  ...
  const handleSubmitForApproval = async (e) => {
    ...
    const payload = {
      ...
      status: (selectedEvent?.status === 'changes_requested' || selectedEvent?.status === 'needs_changes') ? 'pending_supervisor' : (selectedEvent ? selectedEvent.status : 'pending_supervisor'),
      approvalStep: (selectedEvent?.status === 'changes_requested' || selectedEvent?.status === 'needs_changes') ? 0 : (selectedEvent?.approvalStep ?? 0),
      feedback: (selectedEvent?.status === 'changes_requested' || selectedEvent?.status === 'needs_changes') ? undefined : selectedEvent?.feedback,
  ...
  const handleAddSection = () => {
    setForm((prev) => ({
      ...prev,
      customSections: [...(prev.customSections || []), { id: `sec-${Date.now()}-${Math.random().toString(36).slice(2)}`, sectionTitle: '', content: '' }],
    }));
  };
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (!file) return;
                        try {
                          const { filename } = await uploadEventImage(file);
                          setFormField('image', filename);
                          setFormErrors((prev) => ({ ...prev, image: undefined }));
                        } catch (err) {
                          console.warn('Image upload failed', err);
                          setFormErrors((prev) => ({ ...prev, image: err?.data?.error || 'Upload failed' }));
                        }
                      }}
                    />
                    {form.image ? (
                      <div className="rounded-lg border border-slate-200 bg-slate-50/50 overflow-hidden">
                        <div className="relative aspect-[16/9] max-h-24 w-full bg-slate-100">
                          <img src={eventImageUrl(form.image)} alt="Event" className="h-full w-full object-cover" onError={(e) => { e.target.onerror = null; e.target.src = '/manage-events-hero.png'; }} />
                    ...
                    {formErrors.image && <p className="mt-0.5 text-xs text-red-600">{formErrors.image}</p>}
                  </div>
                    ...
                    {(form.customSections || []).length === 0 ? (
                      ...
                      <ul className="space-y-3">
                        {(form.customSections || []).map((sec, index) => (
                          <li key={`section-${index}`} className="rounded-lg border border-slate-200 bg-slate-50/50 p-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {eventsToShow.map((ev) => (
                  <EventCard
                    ...
                    onDeleteDraftConfirm={setDraftToDelete}
                    events={events}
                    onShowFeedback={setFeedbackEvent}
                    isCommunityLeaderOnly={communityLeader && !isSupervisor(user)}
                  />
                ))}
              </div>
            )}
      ...
      {draftToDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-xl p-5">
            <p className="text-sm text-slate-700">Delete this draft event?</p>
            <div className="mt-4 flex gap-3 justify-end">
              <button
                type="button"
                onClick={() => setDraftToDelete(null)}
                className="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-200"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-full bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-400"
              >
                Delete
              </button>
            </div>
